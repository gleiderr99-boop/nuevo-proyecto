...
# --- SOLUCIÓN PARA RENDER: LIMPIEZA TOTAL AL INICIAR ---
# Borra esto solo cuando ya puedas entrar a la página sin errores
...
def gleider_admin():
    ...
    # Lógica de mensajes para el panel
...
def enviar_mensaje(receptor_id):
    if 'user_id' not in session: return redirect(url_for('login'))
    nuevo_m = Mensaje(contenido=request.form.get('mensaje'), emisor_id=session['user_id'], receptor_id=receptor_id)
...
def eliminar_producto(id):
    ...
    if p and (p.user_id == session['user_id'] or session.get('es_admin')):
        db.session.delete(p)
        db.session.commit()
    return redirect(url_for('gleider_admin'))
