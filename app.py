...
# --- LIMPIEZA AUTOMÁTICA (Borra estas 3 líneas cuando el perfil funcione) ---
...
class Producto(db.Model):
    ...
    imagen = db.Column(db.String(200), nullable=True) # Ahora puede ser nulo
    video = db.Column(db.String(200), nullable=True)  # Ahora puede ser nulo
...
def registro():
    if request.method == 'POST':
        ...
        nuevo = User(nombre=request.form['nombre'], correo=correo, telefono=request.form.get('telefono', 'Cliente'),
...
def comentar(p_id):
    if 'user_id' not in session: return redirect(url_for('login'))
    db.session.add(Comentario(contenido=request.form.get('comentario'), user_id=session['user_id'], producto_id=p_id))
    db.session.commit()
